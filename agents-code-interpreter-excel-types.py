...
def process_file(file_path: str, user_messages: list):

    start_time = time.time()
    # Upload a file and add it to the client
    file = project_client.agents.fil upload_file_and_poll(
    ...
    agent = project_client.agents.create_agent(
        ...
        instructions="You are AI Assistant to analyze excel and csv files with images in it. "
    ...
    for user_message in user_messages:
        ...
        for file_path_annotation in all_messages.file_path_annotations:
            print(f"File Paths:")
            ...
            file_name = f"{file_path_annotation.file_path.file_id}_output.csv"
...
# process_file("./data_excel/ExcelwithMacro.xlsm", ["Retrieve the sales data from the Order sheet and combine them into a single table with columns Order Date, Region, Product Name and Sales"])

#process_file("./data_excel/ExcelwithImage.xlsx", ["Retrieve the sum of total sales data from the Consumer Electronics Sheet"])
